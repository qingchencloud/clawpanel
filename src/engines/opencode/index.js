...
async function detectStatus() {
  try {
    ...
    const status = await api.openCodeStatus(getOpenCodePort())
...
export default {
  id: 'opencode',
  name: 'OpenCode',
  description: 'Open Source Coding Agent · Local Web Runtime',
  ...
  getNavItems() {
    return [{
      ...
      items: [
        { route: '/opencode/dashboard', label: t('openCode.dashboard'), icon: 'dashboard' },
        { route: '/opencode/workspace', label: t('openCode.workspace'), icon: 'chat' },
      ],
    }, {
      section: '',
      items: [
  ...
  getRoutes() {
    return [
      { path: '/opencode/dashboard', loader: () => import('./pages/dashboard.js') },
      { path: '/opencode/workspace', loader: () => import('./pages/workspace.js') },
      { path: '/settings', loader: () => import('../../pages/settings.js') },
      { path: '/about', loader: () => import('../../pages/about.js') },
    ]
  },

  getSetupRoute() { return '/opencode/dashboard' },
  getDefaultRoute() { return '/opencode/dashboard' },
