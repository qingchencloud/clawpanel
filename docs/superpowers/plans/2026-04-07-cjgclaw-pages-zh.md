...
**目标：** 实现所有 7 个管理页面（Dashboard、Services、Models、Agents、Gateway、Skills、Extensions），使用 React 19 和 CJGClaw 设计系统 —— 替换现有的 vanilla JS 页面。每个页面使用计划 1 中的 TanStack Query hooks 和 Framer Motion 动画。

**架构：** 每个页面都是一个 React 组件，通过 TanStack Query + Zustand 管理本地状态。页面遵循 CJGClaw 设计规范：青绿色主色（#2C8E65）、深色侧边栏（#1A2E28）、Geist Sans + Noto Sans SC 字体、240px 侧边栏、骨架屏加载、内联错误卡片、状态徽章脉冲动画。

**技术栈：** React 19, TypeScript, TanStack Query v5, Zustand, Framer Motion v11, shadcn/ui, Lucide icons
...
**涉及文件：**
- 修改: `src/pages/dashboard/DashboardPage.tsx`
- 创建: `src/pages/dashboard/components/StatsGrid.tsx`
- 创建: `src/pages/dashboard/components/QuickActions.tsx`
- 创建: `src/pages/dashboard/components/RecentActivity.tsx`

- [ ] **步骤 1：实现带骨架屏加载的 DashboardPage.tsx**
...
**涉及文件：**
- 修改: `src/pages/services/ServicesPage.tsx`
- 创建: `src/pages/services/components/ServiceList.tsx`

- [ ] **步骤 1：实现 ServicesPage.tsx**
...
**涉及文件：**
- 修改: `src/pages/models/ModelsPage.tsx`
- 创建: `src/pages/models/components/ModelCard.tsx`
- 创建: `src/pages/models/components/ModelTestDialog.tsx`

- [ ] **步骤 1：实现 ModelsPage.tsx**
...
**涉及文件：**
- 修改: `src/pages/agents/AgentsPage.tsx`

- [ ] **步骤 1：实现带完整 CRUD 的 AgentsPage.tsx**
...
**涉及文件：**
- 修改: `src/pages/gateway/GatewayPage.tsx`

- [ ] **步骤 1：实现 GatewayPage.tsx**
...
**涉及文件：**
- 修改: `src/pages/skills/SkillsPage.tsx`

- [ ] **步骤 1：实现 SkillsPage.tsx**
...
**涉及文件：**
- 修改: `src/pages/extensions/ExtensionsPage.tsx`

- [ ] **步骤 1：实现 ExtensionsPage.tsx**
...
| 任务 | 页面 | 描述 |
|------|-------|------|
| 1 | Dashboard | 统计网格、快速操作、Agent 概览 |
| 2 | Services | 服务列表，含启动/停止/重启 |
| 3 | Models | 模型卡片，含提供商分组和测试连接 |
| 4 | Agents | 完整 CRUD，含创建/编辑/删除 |
| 5 | Gateway | 状态监控和守护进程信息 |
| 6 | Skills | 已安装列表 + SkillHub 搜索/安装 |
| 7 | Extensions | CfTunnel 和 ClawApp 控制 |

完成此计划后，所有 7 个管理页面均可运行。计划 4 涵盖聊天 + 设置向导页面。